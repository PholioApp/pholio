...
serve(async (req) => {
  ...
  try {
    console.log("Starting promotion checkout process");
    ...
    if (image.seller_id !== user.id) {
      throw new Error("You can only promote your own images");
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `${req.headers.get("origin")}/payment-success?session_id={CHECKOUT_SESSION_ID}&type=promotion`,
